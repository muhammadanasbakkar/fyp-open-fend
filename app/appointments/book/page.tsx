...
function BookPageInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setTherapists(list.map((t: any) => ({ ...t, _id: normalizeId(t?._id) })));
      } catch (e: any) {
        setErr(e.message || "Unable to load therapists");
      } finally {
        setLoadingTherapists(false);
  ...
  // ---------- Load referral (NEW) ----------
  useEffect(() => {
    if (!token || !referralId) { setReferral(null); return; }
    (async () => {
      ...
      try {
        ...
        const data = await api(`api/referrals/${referralId}/packet`, { headers: authHeader(token) }).catch(() => null);
        if (data?.referral) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        let data: any;
        try {
          data = await api(`api/therapist-clinics/therapists/${therapistId}/hospitals`, { headers: authHeader(token) });
        } catch {
          try {
            data = await api(`api/therapistClinics/therapists/${therapistId}/hospitals`, { headers: authHeader(token) });
          } catch {
            data = await api(`api/hospitals/therapist/${therapistId}`, { headers: authHeader(token) });
          }
        }
        const raw = Array.isArray(data) ? data : Array.isArray(data?.hospitals) ? data.hospitals : [];
        const deduped: any = uniqById(raw.map((h: any) => ({ ...h, _id: normalizeId(h?._id) })));
    ...
    (async () => {
      ...
      try {
        ...
        const qs = new URLSearchParams({
          ...
          ...(mode === "in-person" && hospitalId ? { hospital: hospitalId } : {}),
        });
  ...
  async function bookWithTimes(startISO: string, endISO: string) {
    ...
    setMsg("");
    if (!therapistId) return setErr("Please choose a therapist.");
    if (mode === "in-person" && !hospitalId) return setErr("Please select a hospital/clinic.");
    ...
    try {
      ...
      setMsg(`Appointment booked${shouldAttachReferral ? " (linked to referral)" : ""}.`);
    } catch (e: any) {
      setErr(e.message || "Could not create appointment.");
    } finally {
      setPosting(false);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10 space-y-8">
          ...
          {loadingReferral ? (
            ...
            <>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <b>Referral:</b> {referral.status === "active" ? "Active (consented)" :
                  referral.status === "pending-consent" ? "Pending consent" : "Revoked"}
                  {therapistId && normalizeId(referral.toTherapist) !== normalizeId(therapistId) && (
                    <span className="ml-2 text-red-700">
                      • Note: This referral is for a different therapist. Selecting the referred therapist will enable linking.
                    </span>
                  )}
                </div>
                {referral.status === "active" ? (
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className={`flex items-center gap-2 rounded-lg border p-3 ${mode==='in-person'?'ring-2 ring-[var(--brand,#4b7eff)]':''}`}>
            <input type="radio" name="mode" checked={mode === "in-person"} onChange={() => setMode("in-person")} />
            In-person (at a hospital/clinic)
          </label>
          <label className={`flex items-center gap-2 rounded-lg border p-3 ${mode==='online'?'ring-2 ring-[var(--brand,#4b7eff)]':''}`}>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {therapists.map((t) => (
              <button
                key={t._id}
                type="button"
                onClick={() => { setTherapistId(t._id); setMsg(""); setErr(""); }}
                className={`text-left rounded-lg border p-4 hover:bg-gray-50 ${t._id === therapistId ? "ring-2 ring-[var(--brand,#4b7eff)]" : ""}`}
              >
                <div className="flex items-center gap-3">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={t.profilePicture || "/default-avatar.png"} alt={t.name || t.email || "Therapist"} className="h-10 w-10 rounded-full object-cover" />
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium">{t.name || t.email || "Therapist"}</div>
                    {!!t.specializations?.length && (
                      <div className="truncate text-xs text-gray-500">
                        {t.specializations.slice(0, 3).join(", ")}
                        {t.specializations.length > 3 ? "…" : ""}
          ...
          <div className="flex items-end justify-between gap-3">
            <div className="flex-1">
              ...
              {loadingHospitals ? (
                ...
                <Select value={hospitalId} onChange={(e) => setHospitalId(String(e.target.value))}>
                  {hospitals.map((h) => (
                    <option key={h._id} value={h._id}>
                      {h.name}{h.address ? ` — ${h.address}` : ""}
                    </option>
            ...
            <div className="w-44">
              <label className="mb-1 block text-sm text-gray-700">Slot length</label>
          ...
          <div className="mt-4">
            ...
            {loadingSlots ? (
                  ...
                  {slots.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        title={isSelected ? `Selected: ${label}` : `Select ${label}`}
                      >
                        {label}
                      </button>
                    );
                  ...
                  <div className="mt-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                    <div className="text-sm">
                      ...
                      <span className="font-medium">
                        {dayjs(selectedSlot.start).format("ddd, MMM D")} • {dayjs(selectedSlot.start).format("HH:mm")}–{dayjs(selectedSlot.end).format("HH:mm")} ({tz})
                      </span>
                    </div>
                    <div className="flex gap-2">
                      <Button onClick={() => setSelectedSlot(null)}>Change</Button>
                      <Button
                        onClick={confirmSelectedSlot}
                        disabled={posting || !therapistId || (mode === "in-person" && !hospitalId)}
                      >
              ...
              <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                No free slots for this day. Try another date or change slot length.
              </div>
            )}
          </div>
        </div>
