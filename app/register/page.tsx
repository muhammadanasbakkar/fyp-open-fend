...
export default function RegisterPatientPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    setErr(""); setInfo(null); setLoading(true);
    try {
    ...
    <div className="min-h-[calc(100dvh-64px)] bg-gradient-to-b from-white to-gray-50">
      <div className="mx-auto max-w-md px-4 sm:px-6 py-12">
        <div className="mb-4 text-sm text-gray-500">
          <Link href="/" className="hover:underline">Home</Link> <span>›</span> <span>Register (Patient)</span>
        </div>

        <h1 className="text-2xl font-semibold mb-2">Create your patient account</h1>
        <p className="text-sm text-gray-600 mb-6">You’ll receive a unique Patient ID to sign in.</p>
        ...
        {info?.patientId ? (
          <div className="rounded-2xl border border-green-200 bg-green-50 p-6 text-sm text-green-800">
            ...
            <div className="mt-2 flex items-center gap-2">
              <code className="rounded-md bg-white px-2 py-1 text-gray-900 ring-1 ring-green-200">{info.patientId}</code>
              <button
            ...
            <p className="mt-3">
              ...
              <Link href="/login" className="text-[var(--brand,#4b7eff)] underline">login page</Link>.
            </p>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-4">
            <div>
