...
export default function RegisterStaffPage() {
  ...
  // Load hospitals for therapist role
  useEffect(() => {
    if (!therapist) return;
    (async () => {
      try {
  ...
  // Keep primaryHospital consistent
  useEffect(() => {
    if (primaryHospital && !selectedHospitals.includes(primaryHospital)) {
      setPrimaryHospital(selectedHospitals[0] || null);
    }
  ...
  // Clear therapist specific state when switching to receptionist
  useEffect(() => {
    if (!therapist) {
      setSelectedHospitals([]);
      setPrimaryHospital(null);
  ...
  return (
    <div className="min-h-[calc(100dvh-64px)] bg-gradient-to-b from-sky-50/40 via-white to-slate-50">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10 space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-100 bg-sky-50 px-3 py-1 text-[11px] font-medium text-sky-700 shadow-sm">
              ...
              Secure onboarding for therapists and reception staff
            </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white/80 px-4 py-3 text-xs text-slate-600 shadow-sm backdrop-blur md:max-w-xs">
        ...
        <form onSubmit={onSubmit} className="grid gap-6 md:grid-cols-5">
          {/* Main form card */}
          <div className="md:col-span-3 space-y-5 rounded-2xl border border-slate-100 bg-white/90 p-6 shadow-sm backdrop-blur">
            {/* Role switch */}
            <div>
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <span className="text-[11px] text-slate-500">
                  Used to decide access level and workflow
                </span>
              </div>
            ...
            <div className="space-y-3 rounded-xl border border-slate-100 bg-slate-50/60 p-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <Input
                    placeholder="Home or clinic address"
                    value={form.address}
                    onChange={(e) =>
              ...
              <>
                ...
                <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-3">
                    ...
                    <Input
                      type="number"
                      min="0"
                      placeholder="Default in person fee"
                      value={feesInPerson}
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 shadow-sm">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    ...
                    />
                    <Input
                      placeholder="Primary clinic address"
                  ...
                  <label className="mt-3 mb-1 block text-sm text-slate-700">
                    Short bio
                  </label>
                  <textarea
                    className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm focus:border-[var(--brand,#4b7eff)] focus:outline-none focus:ring-1 focus:ring-[var(--brand,#4b7eff)]"
                    rows={3}
                    placeholder="Describe your approach, languages and the type of clients you support."
                    value={form.bio}
                    onChange={(e) =>
                  ...
                  <div className="mt-4 grid gap-4">
                    <Fieldset
                      ...
                      items={MODALITIES}
                      values={modalities}
                      onToggle={(v) => toggle(modalities, setModalities, v)}
                    />
                    <Fieldset
                      title="Concerns"
                      items={CONCERNS}
                    ...
                    <Fieldset
                      ...
                      items={POPULATIONS}
                      values={populations}
                      onToggle={(v) => toggle(populations, setPopulations, v)}
                    />
                    <Fieldset
                      title="Care settings"
                      items={CARE}
                ...
                {/* Hospitals and per hospital pricing */}
                <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-slate-900">
                  ...
                  {selectedHospitals.length > 0 && (
                    <p className="mt-2 text-xs text-slate-600">
                      Selected clinics {selectedHospitals.length}.{" "}
                      {primaryHospital
                        ? "Primary clinic selected."
                        : "Please set one primary clinic."}
                    </p>
                  )}
                </div>
              </>
          ...
          <aside className="md:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-100 bg-white/90 p-6 shadow-sm backdrop-blur">
              ...
              {profilePreview ? (
                ...
                <div className="mb-3 grid h-28 w-28 place-items-center rounded-xl bg-slate-100 text-xs text-slate-500">
                  No photo selected
                </div>
              )}
              ...
              />
              <p className="mt-1 text-[11px] text-slate-500">
                Use a clear headshot. JPG or PNG, around 3 to 5 MB.
              </p>
            </div>
