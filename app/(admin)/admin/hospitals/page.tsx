  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        ...
        setList((prev) => prev.map((h) => (h._id === editing._id ? { ...h, ...updated } : h)));
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10 space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="w-full sm:w-72">
          <Input placeholder="Search name, city, address…" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <p className="text-sm font-medium">{editing ? "Edit hospital" : "Add a hospital"}</p>
          {editing && (
            <button
              onClick={resetForm}
        ...
        <form onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g., City Care Hospital" />
          </div>
          ...
          <div>
            <label className="mb-1 block text-sm text-gray-700">Latitude (optional)</label>
            <Input value={lat} onChange={(e) => setLat(e.target.value)} placeholder="e.g., 31.5204" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-gray-700">Longitude (optional)</label>
            <Input value={lng} onChange={(e) => setLng(e.target.value)} placeholder="e.g., 74.3587" />
          </div>

        <div className="sm:col-span-2">
            <Button disabled={saving}>{saving ? "Saving…" : editing ? "Save changes" : "Create hospital"}</Button>
          </div>
        </form>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          ...
          <p className="text-xs text-gray-500">{filtered.length ? `${filtered.length} total` : "None yet"}</p>
        </div>
        ...
        {loadingList ? (
          ...
          <div className="space-y-3">
            {filtered.map((h) => (
              <div key={h._id} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                ...
                {h.description && (
                  <p className="mt-2 text-sm text-gray-700 whitespace-pre-wrap">{h.description}</p>
                )}
              </div>
            ))}
          </div>
