...
export default function PendingUsersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((u) => {
      ...
      const hay = [
        u.name, u.email, u.role, u.phone, u.address, u.cnic,
    ...
    <div className="min-h-[calc(100dvh-64px)] bg-gradient-to-b from-white to-gray-50">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10">
        <div className="mb-4 text-sm text-gray-500">
          ...
          <span>Admin</span> <span>›</span> <span>Pending users</span>
        </div>

        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Pending Users</h1>
            <p className="text-sm text-gray-600">Approve or reject therapists and receptionists.</p>
          </div>
          <Link href="/dashboard" className="text-sm text-[var(--brand,#4b7eff)] hover:underline">Back to dashboard</Link>
        </div>
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          <Input placeholder="Search name/email/cnic/specialization" value={q} onChange={(e) => setQ(e.target.value)} />
        ...
        {loading ? (
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">Loading…</div>
        ) : filtered.length === 0 ? (
          ...
          <ul className="grid gap-4">
            {filtered.map((u) => (
              <li key={u._id} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="h-12 w-12 overflow-hidden rounded-xl ring-1 ring-gray-200">
                      ...
                      <img
                        alt={u.name}
                        src={u.profilePicture || "/avatar-placeholder.png"}
                        className="h-full w-full object-cover"
                      />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-900">
                        {u.name} <span className="ml-2 rounded-md bg-gray-100 px-2 py-0.5 text-xs">{u.role}</span>
                      </p>
                      <p className="text-sm text-gray-600">
                        {u.email} {u.phone ? <>• {u.phone}</> : null} {u.cnic ? <>• CNIC: {u.cnic}</> : null}
                      </p>
                      {u.address && <p className="text-xs text-gray-500 mt-1">{u.address}</p>}
                          ...
                          {!!u.therapistInfo?.certifications?.length && (
                            <div className="text-xs text-gray-700">
                              ...
                              <div className="mt-1 flex flex-wrap gap-2">
                                {u.therapistInfo.certifications.slice(0, 4).map((c, i) => (
                                  <a
                                    key={i}
                                    href={c.fileUrl}
                                    target="_blank"
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <Button onClick={() => act(u._id, "approve")}>Approve</Button>
                    <button
